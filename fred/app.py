...
def _macro_prewarm():
    ...
    if not FRED_AVAILABLE and ({'monetario_usa', 'yields_usa'} - set(_MACRO_CACHE)):
        print("⚠ [macro-cache] fredapi NON installato: i dataset USA (Analisi Monetaria, "
              "Curva dei Tassi) non sono in cache e non si possono scaricare — "
              "installa 'fredapi' (è in requirements.txt)", flush=True)
...
def aggiorna(n_clicks, api_key, upload_meta, source_type, eur_geo):
    ...
    # All'apertura della pagina (nessun click) il tab si carica DA SOLO, ma solo
    # se il dato è già nella cache settimanale: mai un download live automatico.
    # Senza questa guardia il tab partiva comunque a scaricare e, se FRED non
    # rispondeva, restava l'errore "Nessun dato" al posto dei grafici.
    if not n_clicks:
        if upload_meta:
            raise PreventUpdate           # gli xlsx caricati si aprono solo su richiesta
        _geo = eur_geo or "EA20"
        if source_type == "both":
            _attesa = ('monetario_usa', f'monetario_eur_{_geo}')
        elif source_type == "eur":
            _attesa = (f'monetario_eur_{_geo}',)
        else:
            _attesa = ('monetario_usa',)
...
# ── Scheduler settimanale: ricostruisce la cache ogni domenica sera alle 22:00
#    Europe/Rome (a settimana chiusa ci sono tutti i rilasci FRED/Eurostat, e a
#    quell'ora nessuno sta usando il sito). Stesso strumento già usato in
#    portafoglio/app.py. Con gunicorn workers=1 gira una sola volta.
try:
    ...
    _macro_scheduler.add_job(
        lambda: _build_macro_cache(reason='schedulato dom 22:00'),
    ...
    print("✓ [macro-cache] scheduler settimanale attivo (dom 22:00 Europe/Rome)", flush=True)
except Exception as _e:
